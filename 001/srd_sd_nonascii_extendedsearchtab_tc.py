...
class SrdSdNonasciiExtendedsearchtabTc(unittest.TestCase):
    ...
    def test_srd_sd_nonascii_extendedsearchtab_tc(self):
        driver = self.driver
        driver.get(self.base_url + "SRD/search.php?extended")
        ...
        self.assertEqual("Test Case ID", driver.find_element_by_xpath("//div[@id='content']/form/table/tbody/tr/td[2]").text)
        self.assertEqual("Description", driver.find_element_by_xpath("//div[@id='content']/form/table/tbody/tr/td[7]").text)
        self.assertEqual("Weakness", driver.find_element_by_xpath("//div[@id='content']/form/table/tbody/tr/td[8]").text)
        driver.back()
        driver.find_element_by_xpath("//input[@name='description']").send_keys(u"ñó? ä?çíì ??/??  ??/?? Huáy?; ?? Zh?ngwén ???????? Lech Wa??sa æøå")
        driver.find_element_by_xpath("//input[@name='Submit']").click()
        self.assertEqual("SAMATE Reference Dataset :: View all test cases", driver.title)
        self.assertEqual("View/Download Downloads:      \n \n There is no such test case in the database! Back to the previous page", driver.find_element_by_xpath("//div[@id='content']").text)    
        self.assertEqual("Back to the previous page", driver.find_element_by_link_text("Back to the previous page").text)
        driver.back()
